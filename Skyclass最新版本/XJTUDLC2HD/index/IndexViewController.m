...
- (void)viewDidLoad {
    ...
    float AD_height = 300;//广告栏高度
    UICollectionViewFlowLayout *flowLayout=[[UICollectionViewFlowLayout alloc] init];
    [flowLayout setScrollDirection:UICollectionViewScrollDirectionVertical];
    flowLayout.headerReferenceSize = CGSizeMake(fDeviceWidth, AD_height+40);//头部
    ...
    //[self.RefreshControl addTarget:self action:@selector(getOpenCourseVideoData) forControlEvents:UIControlEventValueChanged];
   ...
   // self.collectionView
     ...
     ***广告栏
     */
    _headerView = [[AdvertisingColumn alloc]initWithFrame:CGRectMake(0, 0, fDeviceWidth, AD_height+30)];
...
-(void)initHeadView{
    ...
    for (int i =0; i<[headVideoArray count]; i++) {
       ...
       CGRect rect = CGRectMake(fDeviceWidth*i,0, fDeviceWidth, 300);
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(![[dic objectForKey:@"VideoPicUrl"] isEqualToString:@""]){
        ...
        /*//         NSLog(@"%@----------",[dic objectForKey:@"VideoPicUrl"]);
        NSURL *url = [NSURL URLWithString:[dic objectForKey:@"VideoPicUrl"]];
        NSLog(@"url:%@",url);
        NSData *resultData = [NSData dataWithContentsOfURL:url];
        UIImage *img = [UIImage imageWithData:resultData];
