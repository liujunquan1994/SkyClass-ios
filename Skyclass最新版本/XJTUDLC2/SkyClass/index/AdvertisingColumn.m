...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        //分页控制
        _pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(50, 0, CGRectGetWidth(containerView.frame)-100, 20)];
...
-(void)timerAction:(NSTimer *)timer{
    if (_totalNum>1) {
        ...
        _imageNum.text = _slideText[index];
...
- (void)setArray:(NSArray *)imgArray textArray:(NSArray *)text urlArray:(NSArray *)url{
    ...
    if (_totalNum>0) {
        ...
        _imageNum.text = text[_pageControl.currentPage];
...
-(void)ClickEventOnImage:(id)sender{
  ...
  //  NSString * urlString =[dic objectForKey:@"VideoUrl"];
    NSURL * movieUrl = [NSURL URLWithString:_urlArray[tag]];
    NSLog(@"movieUrl:%@",movieUrl);
    _movieplayer = [ [ MPMoviePlayerViewController alloc]initWithContentURL:movieUrl];
    _movieplayer.view.transform = CGAffineTransformConcat(_movieplayer.view.transform, CGAffineTransformMakeRotation(M_PI_2));
    //self presentM
    [self.superclass presentMoviePlayerViewControllerAnimated:_movieplayer];
    NSLog(@"tag:%d",tag);
