    ...
    }else if([self checkNetwork:[self getTeacherString]])
    {
        ...
        if ([self getTeacherURL]) {
           ...
           */
            ...
            videoPlayerVC.url = [self getTeacherURL];
            videoPlayerVC.isOnline = YES;
...
- (IBAction)playerScreen:(id)sender {
...

    {
        ...
        if ([self getScreenURL]) {
            ...
            UIStoryboard * VideoPlayerStoryboard = [UIStoryboard storyboardWithName:@"VideoPlayer" bundle:nil];
            VideoPlayerVC  *videoPlayerVC = [VideoPlayerStoryboard instantiateViewControllerWithIdentifier:@"VideoPlayer"];
            videoPlayerVC.url = [self getScreenURL];
            videoPlayerVC.isOnline = YES;
            [self presentViewController:videoPlayerVC animated:YES completion:^{
...
-(void)showDetail{
    ...
    NSLog(@"%@",[[NSString alloc]initWithFormat:@"http://%@:8134/hls-live/%@/_definst_/%@/%@.m3u8",_hlsMsg.serverpath,_hlsMsg.location,_hlsMsg.screenpath,_hlsMsg.screenpath]);
...
{
    //KRVideoPlayerController *player = _movieplayer.moviePlayer;
...
-(void)removeMovieNotificationHandlers
{
    ...
    [[NSNotificationCenter defaultCenter]removeObserver:self name:MPMediaPlaybackIsPreparedToPlayDidChangeNotification object:self.videoController];
    [[NSNotificationCenter defaultCenter]removeObserver:self name:MPMoviePlayerPlaybackStateDidChangeNotification object:self.videoController];
}
